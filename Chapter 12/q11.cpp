...
Vec<Str> other_split(const Str& str)
{
...
int main()
{
    ...
    Vec<Str> f1 = frame(split(s1));
    Vec<Str> f2 = frame(other_split(s2));

    Vec<Str> v = vcat(f1, f2);
    Vec<Str> h = hcat(f1, f2);

    for (Vec<Str>::size_type i = 0; i != v.size(); ++i)
        cout << v[i] << endl;

    for (Vec<Str>::size_type i = 0; i != h.size(); ++i)
        cout << h[i] << endl;
